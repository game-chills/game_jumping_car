...
function timer_set_timeout(_callback, _milliseconds) {
	static _memory = __timers_memory();
	
	var _id = _memory.timeout_id_counter++;
	var _t_milliseconds;
	...
	array_push(_memory.timeouts_async, _data);
	return _r_id
}
...
function timer_set_timeout_sync(_callback, _frames) {
	...
	array_push(_memory.timeouts_sync, _data);
	return _r_id
}
...
function timer_clear(_id) {
	...
	if (_kind == "timeout") {
		...
		var _array = 
			_kind == "sync" 
			? _memory.timeouts_sync
			: _memory.timeouts_async;
...
function timer_update() {
	...
	var _timeouts_sync_count = array_length(_memory.timeouts_sync);
	
	for (i = 0; i < _timeouts_sync_count; ++i) {
		_data = _memory.timeouts_sync[i];
	...
	/// TODO: swap
	array_resize(_memory.timeouts_sync, 0);
	_memory.timeouts_sync = _update_timeouts_sync;
	
	/* timeouts async */
	var _update_timeouts_async = [];
	var _timeouts_async_calls = [];
	var _timeouts_async_count = array_length(_memory.timeouts_async);
	var _async_time = current_time;
	var _async_dead_time;
	
	for (i = 0; i < _timeouts_async_count; ++i) {
		_data = _memory.timeouts_async[i];
	...
	/// TODO: swap
	array_resize(_memory.timeouts_async, 0);
	_memory.timeouts_async = _update_timeouts_async;
	...
	_count = array_length(_update_timeouts_async);
	for (i = 0; i < _count; ++i) {
		_data = _update_timeouts_async[i];
		_data.callback({
			callback: _data.callback,
		});
	}
